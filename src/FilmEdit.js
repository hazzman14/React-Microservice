...
class FilmEdit extends Component {
    ...
    async componentDidMount() {
        if (this.props.match.params.id !== 'new') {
            const film = await (await fetch(`http://18.168.164.47:8181/api/films/${this.props.match.params.id}`)).json();
    ...
    render() {
        const {item} = this.state;
        const title = <h2>{item.id ? 'Edit Film' : 'Edit Film'}</h2>;
                ...
                <Form onSubmit={this.handleSubmit}>
                    ...
                    <FormGroup>
                        ...
                        <Input type="number" name="lengthMinutes" id="lengthMinutes" value={item.lengthMinutes || ''}
                               onChange={this.handleChange} autoComplete="lengthMinutes"/>
                    </FormGroup>
                    <FormGroup>
                        <Label for="releaseYear">Release Year</Label>
                        <Input type="number" name="releaseYear" id="releaseYear" value={item.releaseYear || ''}
                               onChange={this.handleChange} autoComplete="releaseYear"/>
                    </FormGroup>
                    <FormGroup>
